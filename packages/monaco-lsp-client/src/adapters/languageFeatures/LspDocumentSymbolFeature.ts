  ...
  constructor(private readonly _connection: LspConnection) {
    ...
    this._register(
      this._connection.capabilities.registerCapabilityHandler(
        capabilities.textDocumentDocumentSymbol,
        true,
        (capability) => {
    ...
    if (Array.isArray(result) && result.length > 0) {
      ...
      if ("location" in first) {
        ...
        return (result as SymbolInformation[]).map((symbol) =>
          toMonacoSymbolInformation(symbol, this._client),
        );
      }

      // DocumentSymbol[]
      return (result as DocumentSymbol[]).map((symbol) =>
        toMonacoDocumentSymbol(symbol, this._client, translated.textDocument),
      );
    }
...
function toMonacoDocumentSymbol(
  ...
  return {
    ...
    tags:
      symbol.tags
        ?.map((tag) => toMonacoSymbolTag(tag))
        .filter((t): t is monaco.languages.SymbolTag => t !== undefined) || [],
    ...
    children:
      symbol.children?.map((child) => toMonacoDocumentSymbol(child, client, textDocument)) || [],
  };
}
  ...
  return {
    ...
    tags:
      symbol.tags
        ?.map((tag) => toMonacoSymbolTag(tag))
        .filter((t): t is monaco.languages.SymbolTag => t !== undefined) || [],
    range: client.bridge.translateBackRange({ uri: symbol.location.uri }, symbol.location.range)
      .range,
    selectionRange: client.bridge.translateBackRange(
