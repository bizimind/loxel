...
import type { SelectionRange, SelectionRangeRegistrationOptions } from "../../types";
  ...
  constructor(private readonly _connection: LspConnection) {
    ...
    this._register(
      this._connection.capabilities.addStaticClientCapabilities({
        textDocument: { selectionRange: { dynamicRegistration: true } },
      }),
    );

    this._register(
      this._connection.capabilities.registerCapabilityHandler(
        capabilities.textDocumentSelectionRange,
        true,
        (capability) => {
