...
export function toLspCodeActionTriggerKind(
  ...
  return (
    monacoCodeActionTriggerTypeToLspCodeActionTriggerKind.get(monacoTrigger) ??
...
export function toMonacoCompletionItemKind(
  ...
  return (
    lspCompletionItemKindToMonacoCompletionItemKind.get(kind) ??
...
export function toMonacoCompletionItemTag(
  tag: CompletionItemTag,
): monaco.languages.CompletionItemTag | undefined {
...
export function toLspCompletionTriggerKind(
  ...
  return (
    monacoCompletionTriggerKindToLspCompletionTriggerKind.get(monacoKind) ??
...
export function toMonacoInsertTextRules(
...
export function toMonacoDocumentHighlightKind(
  ...
  return (
    lspDocumentHighlightKindToMonacoDocumentHighlightKind.get(kind) ??
...
export function toMonacoFoldingRangeKind(
...
export function toMonacoDiagnosticSeverity(
...
export function toLspSignatureHelpTriggerKind(
  monacoKind: monaco.languages.SignatureHelpTriggerKind,
): SignatureHelpTriggerKind {
  return (
    monacoSignatureHelpTriggerKindToLspSignatureHelpTriggerKind.get(monacoKind) ??
...
export function toMonacoCommand(
...
export function toMonacoInlayHintKind(
...
export function toMonacoLanguageSelector(
  ...
  return s.map<monaco.languages.LanguageFilter>((s) => {
    if ("notebook" in s) {
      ...
      return {
        notebookType: s.notebook.notebookType,
        language: s.language,
        pattern: s.notebook.pattern,
        scheme: s.notebook.scheme,
      };
    }
...
export function matchesDocumentSelector(
  ...
  for (const filter of selector) {
    ...
    if (filter.scheme && filter.scheme !== uri.scheme) continue;
    if (filter.pattern && !picomatch.isMatch(uri.path, filter.pattern)) continue;
    return true;
  }
  return false;
}
export function toDiagnosticMarker(diagnostic: Diagnostic): monaco.editor.IMarkerData {
  ...
  if (diagnostic.tags) {
    marker.tags = diagnostic.tags
      .map((tag) => toMonacoDiagnosticTag(tag))
