  ...
  constructor(private readonly _connection: LspConnection) {
    ...
    this._register(
      this._connection.connection.registerRequestHandler(
        api.client.workspaceInlayHintRefresh,
        async () => {
    ...
    this._register(
      this._connection.capabilities.registerCapabilityHandler(
        capabilities.textDocumentInlayHint,
        true,
        (capability) => {
...
class LspInlayHintsProvider implements monaco.languages.InlayHintsProvider {
  ...
  constructor(
    ...
    if (_capabilities.resolveProvider) {
      this.resolveInlayHint = async (
        hint: ExtendedInlayHint,
        ...
        if (resolved.textEdits) {
          hint.textEdits = resolved.textEdits.map((edit) => {
            ...
            return { range: translated.range, text: edit.newText };
          });
        }
...
async function retryOnContentModified<T>(cb: () => Promise<T>): Promise<T> {
  const nRetries = 3;
  let triesLeft = nRetries;
  while (true) {
    try {
      return await cb();
    } catch (e) {
      if (e instanceof Error && e.message === "content modified" && triesLeft > 0) {
        triesLeft--;
        continue;
      }
      throw e;
