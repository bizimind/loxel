...
import type { DocumentOnTypeFormattingRegistrationOptions } from "../../types";
  ...
  constructor(private readonly _connection: LspConnection) {
    ...
    this._register(
      this._connection.capabilities.addStaticClientCapabilities({
        textDocument: { onTypeFormatting: { dynamicRegistration: true } },
      }),
    );

    this._register(
      this._connection.capabilities.registerCapabilityHandler(
        capabilities.textDocumentOnTypeFormatting,
        true,
        (capability) => {
