...
- `browser:authenticate-in-chrome` — a browser panel asks the main process to run the isolated Chrome authentication helper described below. Cookie and DevTools Protocol data never cross the preload boundary.

**Chrome authentication helper (macOS and Linux)**: Browser panels have an **Authenticate in Chrome** toolbar action for sign-in flows, such as phone-passkey QR authentication, that Google Chrome supports but Electron's embedded Chromium does not. It launches the installed Google Chrome (`/Applications` or `~/Applications` on macOS; `google-chrome`, `chromium` and friends on `PATH` or `/opt/google/chrome` on Linux; `LOXEL_CHROME_PATH` overrides discovery on any platform) with a new private temporary profile. The helper never opens or reads the user's normal Chrome profile. After the user finishes signing in and confirms the native Loxel dialog, the main process copies only supported cookies applicable to the original and final sites into the shared persistent `persist:browser` Electron partition and navigates the Loxel tab to Chrome's final URL.

On Linux a snap-packaged Chromium is not supported: it runs in a private `/tmp` namespace and cannot see the temporary profile. Point `LOXEL_CHROME_PATH` at a regular Chrome or Chromium build instead. The helper does not import passwords, extensions, browser history, or other profile data. Partitioned (CHIPS) cookies are skipped because Electron's public cookie setter does not preserve their partition key. Session cookies work for the current Loxel session but may not survive an app restart. Loxel removes the temporary profile on success, cancel, window close, and app quit; an OS-level hard crash can leave that temporary directory for the operating system to clean up.
